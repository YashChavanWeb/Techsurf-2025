...
//   const projectUid = process.env.NEXT_PUBLIC_CONTENTSTACK_PROJECT_ID as string;
//     const edgeApiUrl = process.env.NEXT_PUBLIC_CONTENTSTACK_REGION === 'EU' 
//         ? 'https://eu-personalize-edge.contentstack.com' 
//         : 'https://us-personalize-edge.contentstack.com'; 


// Personalize.setEdgeApiUrl(edgeApiUrl)
// Personalize.init(projectUid)

// export const PersonalizeContext = createContext<typeof Personalize | null>(null);
...
// helper functions to retrieve Contentstack endpoints
import { getContentstackEndpoints, getRegionForString } from "@timbenniks/contentstack-endpoints";
import { createContext } from "react";
import Personalize from "@contentstack/personalize-edge-sdk";
...
export async function getPage(url: string, variantParam: string) {
  ...
  const pageQuery = await stack.contentType("page").entry();
  pageQuery.addParams({ include_all: true });
  pageQuery.addParams({ include_dimension: true });
  pageQuery.addParams({ include_applied_variants: true });
